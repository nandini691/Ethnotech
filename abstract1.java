...
class Demo {
    public static void main(String args[]) {
        ...
        if(item_no == 1) {
            Biryani b = new Biryani("Nandini", "kothakota");
            b.display();
            b.order();
        }
        else if(item_no == 2) {
            Idly i = new Idly("Nani", "ghattu");
            i.display();
            i.order();
        }
        else if(item_no == 3) {

            Chapathi c = new Chapathi("Nandhu", "bangalore");
            c.display();
            c.order();
        }
        else if(item_no == 4) {
            Puri p = new Puri("bhavya", "Kerala");
            p.display();
            p.order();
        }
        else if(item_no == 5) {
            Maggie m = new Maggie("manju", "Hyderabad");
            m.display();
            m.order();
        }
        else if(item_no == 6) {
            Eggrice e = new Eggrice("charan", "Chennai");
            e.display();
            e.order();
        }
        else if(item_no == 7) {
            Paneer p = new Paneer("bharath", "Delhi");
            p.display();
            p.order();
        }
        else if(item_no == 8) {
            Burger b = new Burger("vani", "Mumbai");
            b.display();
            b.order();
        }
        else if(item_no == 9) {
            Palav p = new Palav("loku", "Pune");
            p.display();
            p.order();
        }
        else if(item_no == 10) {
            Icecream i = new Icecream("mahi", "Goa");
            i.display();
            i.order();
        }
        else {
